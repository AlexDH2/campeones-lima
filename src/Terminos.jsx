...
export default function Terminos() {
  ...
  const [estado, setEstado] = useState('Cargando términos y políticas…');
  useEffect(() => {
    let activo = true;
    supabase.from('configuracion_web').select('valor').eq('clave','terminos_condiciones').maybeSingle().then(({data,error}) => {
      if (!activo) return;
      if (error) setEstado('No se pudieron cargar los términos. Recarga la página para reintentar.');
      else { setDatos(data?.valor || null); setEstado('Los términos y políticas aún no se han publicado. Consulta las condiciones por WhatsApp antes de inscribirte.'); }
    });
    return () => { activo = false; };
  }, []);
